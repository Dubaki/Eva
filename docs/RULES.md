...
При переносе файла задачи в `tasks/done/` **обязательно** дописать в конец файла блок завершения:
...
### Сделано
- [что фактически выполнено; отличия от постановки — явно]

### Изменённые файлы
- `path/to/file.ts` — что изменено (1 строка на файл)
...
**Правила:**
- Блок обязателен для всех задач типа `feat`, `fix`, `hotfix`, `emergency`.
- Для `docs` и `db`-миграций достаточно первых 4 полей + коммит.
- Поле «Побочные эффекты / риски» **не оставлять пустым** — писать «нет» явно.
- Нумерация следующей задачи: `max(существующих номеров) + 1`, никогда вручную. Перед созданием выполнить `ls tasks/done/ tasks/todo/ | sort` чтобы не повторить номер после потери контекста.
