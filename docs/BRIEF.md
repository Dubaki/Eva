...
> Версия: 1.0 | Дата: 08.04.2026 | Статус: **Refining Requirements & Core Logic**
...
- **SDK:** Telegram Mini Apps SDK (интеграция подтверждена)
...
7. **[Task 026]** Оптимизация UX (кнопка Назад) и исправление логики гостевого доступа

---

## 🔭 Scope / Features

### Обязательные функции (Пункт 19 ТЗ)

- [ ] **Проверка подписки на канал** — перед началом теста пользователь должен быть подписан на Telegram-канал (Done/Check)
- [ ] **Сохранение результатов пользователя в БД** — каждый пройденный тест сохраняется в Supabase (`test_results`)
- [ ] **Реферальная система** — Unlock Результата №2 (вторичная опора) доступен после приглашения минимум 2 друзей
- [ ] **Прогресс прохождения теста** — визуальный индикатор (progress bar) с номером текущего вопроса
- [ ] **Кулдаун и напоминание через 2 месяца** — ограничение повторного прохождения, уведомление через бота
- [ ] **Admin/Content management** — возможность добавления и редактирования контента (вопросы, тексты, ссылки)
