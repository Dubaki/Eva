...
1. **`LOGIC.md`** (в папке docs\base\) — полная логика всех страниц, сообщений бота, БД, переходов. **Главный источник правды по поведению.**
2. **`TEXTS.md`** (в папке docs\base) — все тексты слово-в-слово. **Главный источник правды по формулировкам.**
...
Эти три файла — закон. Если в коде ты найдёшь что-то, что им противоречит — приоритет всегда у этих документов.
...
| `app/api/subscription/confirm/route.ts` | Использовать `process.env.NEXT_PUBLIC_APP_URL` вместо хардкода `eva-9udm.vercel.app`. Текст из `TEXTS.bot.subscriptionConfirmed`. Слать сообщение **только если** `wasAlreadySubscribed === false`. |
...
- `LOGIC.md`, `TEXTS.md`, `design_reference.html` — без изменений.
